...
async def test_사용자가_변경하는_항목만_변경되고_나머지는_기존_값을_유지한다(
    client_with_auth: TestClient, payload: dict, host_user: User
):
    # 현재 사용자 정보를 보관한다.
...
async def test_최소_하나_이상_항목을_변경해야_하며_그렇지_않으면_오류를_일으킨다(client_with_auth: TestClient):
    resopnse = client_with_auth.patch("/account/@me", json={})
